 ...
 make info
 make image PROFILE=phicomm_psg1218a PACKAGES="uhttpd uhttpd-mod-ubus libiwinfo-lua luci-base  luci-mod-admin-full luci-theme-bootstrap wpad-openssl luci-i18n-base-zh-cn luci-i18n-firewall-zh-cn luci iptables jsonfilter ipset  dnsmasq-full coreutils coreutils-nohup bash curl ca-certificates ip-full iptables-mod-tproxy luci-compat iptables-mod-extra libcap libcap-bin ruby ruby-yaml kmod-tun unzip -wpad-basic-wolfssl -ip6tables -odhcp6c -kmod-ipv6 -kmod-ip6tables -odhcpd-ipv6only" CONFIG_IPV6=n
